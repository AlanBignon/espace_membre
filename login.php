    ...
    if(!empty($tableaucsv)) {
        ...
        foreach ($tableaucsv[$i] as $donnes){
            ...
            if ($_SESSION["libre"] = true) {
                if (isset($_POST["email"]) && $tableaucsv[$i][$p] == $_POST["email"] && isset($_POST["password"]) && $tableaucsv[$i][$p+1] == $_POST["password"]){
                    $_SESSION["log"] = true;
                    redirectionDashboard();
                } elseif (isset($_POST["email"]) && $tableaucsv[$i][$p] != $_POST["email"]) {
                    echo "Email non enregistré";
                    break;
                }elseif (isset($_POST["password"]) && $tableaucsv[$i][$p+1] != $_POST["password"]) {
